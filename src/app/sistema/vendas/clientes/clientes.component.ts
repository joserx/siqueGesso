...
export class ClientesComponent implements OnInit {
  ...
  public toggleTipoPessoa(value: string): void {
    ...
    if (value == 'fisica') {
      ...
      this.clienteForm.controls.cpf.setValidators([
        Validators.required,
        BrazilValidator.isValidCpf(),
      ]);
      this.clienteForm.controls.rg.setValidators([
        Validators.required,
        BrazilValidator.isValidRG(),
      ]);
      ...
      this.clienteForm.controls.email.setValidators([
        Validators.required,
        Validators.email,
      ]);
      ...
      // Company validators
      this.clienteForm.controls.fantasyName.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.socialReason.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.subscription.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.cnpj.setValidators([
        Validators.required,
        BrazilValidator.isValidCpf(),
      ]);
      this.clienteForm.controls.companyCellphone.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.companyTelephone.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.birthDateCompany.setValidators([
        Validators.required,
      ]);
      this.clienteForm.controls.ramal.setValidators([Validators.required]);
      this.clienteForm.controls.companyEmail.setValidators([
        Validators.required,
        Validators.email,
      ]);
  ...
  changeAddress(event: any, i: number) {
    ...
    this.correiosService.consultaCep(cep).subscribe((data: any) => {
      if (data.cep) {
        (this.clienteForm.controls.addresses as FormArray)
          .at(i)
          .get('street')
  ...
  submitClient(data: any) {
    if (this.clienteForm.valid) {
      ...
      methodsChecked.forEach((id: any) => {
        ...
        data.condicoesPagamento = condicoesSelecionadas;
    ...
    if (button.checked) {
      ...
      for (let control of this.tabela.value) {
        if (control.nome == button.value) {
          this.tabela.controls.splice(
            this.tabela.controls
              .map(function (e: any) {
                return e.nome;
              })
              .indexOf(button.value),
            1
          );
          this.tabela.value.splice(
            this.tabela.value
              .map(function (e: any) {
                return e.nome;
              })
